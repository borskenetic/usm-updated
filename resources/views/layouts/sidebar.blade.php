...
@php
    ...
    if ($shellUser && $moduleAccess->isDeveloper($shellUser)) {
        ...
        $commandLinks = collect(match ($shellActiveModule) {
            \App\Services\Auth\ModuleAccessService::SUPER_ADMIN => [
                ['label' => 'Super Admin Dashboard', 'route' => 'dashboard.super-admin', 'icon' => 'bi-shield-lock', 'group' => 'Dashboard'],
                ['label' => 'Staff Accounts', 'route' => 'users.index', 'icon' => 'bi-people', 'group' => 'Staff'],
                ['label' => 'Create Staff', 'route' => 'users.create', 'icon' => 'bi-person-plus', 'group' => 'Staff'],
            ],
            \App\Services\Auth\ModuleAccessService::LIBRARY => array_values(array_filter([
                ['label' => 'Library Dashboard', 'route' => $moduleAccess->hasLibraryAdminAccess($shellUser) ? 'dashboard.library-admin' : 'dashboard.library-staff', 'icon' => 'bi-book', 'group' => 'Dashboard'],
                ...
                $moduleAccess->hasLibraryAdminAccess($shellUser)
                    ? ['label' => 'Activity log', 'route' => 'library.attendance.activities', 'icon' => 'bi-activity', 'group' => 'Utilities']
                    : null,
            ])),
            \App\Services\Auth\ModuleAccessService::ATTENDANCE => array_values(array_filter([
                ['label' => 'Attendance Dashboard', 'route' => $moduleAccess->hasAttendanceAdminAccess($shellUser) ? 'dashboard.attendance-admin' : 'dashboard.attendance-staff', 'icon' => 'bi-clock-history', 'group' => 'Dashboard'],
                ...
                $moduleAccess->hasAttendanceAdminAccess($shellUser)
